...
st.header(":blue[BAB 4 PENDIDIKAN - JUMLAH GURU]")
st.subheader("", divider='rainbow')

datakk = pd.read_excel("data/jumlah_guru.xlsx")
sort_datakk = datakk.sort_values(by=['tahun', 'namakab', 'namakec', 'namadesa'], ascending=[False,True,True,True])
sekolah_kec = sort_datakk.groupby(['namakab', 'namakec', 'namadesa','tahun'])[['jumlah_gurusd', 'jumlah_gurusmp', 'jumlah_gurusma']].sum().reset_index()

pilihankab = sort_datakk['namakab'].unique()

pilihantahun = sort_datakk['tahun'].unique()

pilihanjenjang = ['jumlah_gurusd', 'jumlah_gurusmp', 'jumlah_gurusma']
...
with st.container(border=True):
    ...
    tabelseri = tabelseri[['tahun', 'namakab', 'namakec', 'namadesa', 'jumlah_gurusd', 'jumlah_gurusmp', 'jumlah_gurusma']]
